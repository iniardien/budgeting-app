...
<body class="min-h-screen bg-[radial-gradient(circle_at_top,_#dbeafe,_#f8fafc_55%)] text-slate-900 antialiased">
    <main class="mx-auto flex min-h-screen w-full max-w-6xl items-center justify-center px-4 py-10">
        <div
            class="grid w-full overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-200/70 lg:grid-cols-[1.1fr_minmax(24rem,30rem)]">
            <section class="hidden bg-blue-600 px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between">
                <div>
                    <div class="flex h-14 w-14 items-center justify-center rounded-3xl bg-white/15 text-lg font-bold">B
                    </div>
            ...
            <section class="px-6 py-8 sm:px-10 sm:py-12">
                <div class="mx-auto w-full max-w-md">
                    <div class="mb-8">
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-3xl bg-blue-600 text-base font-bold text-white">
                            B</div>
                    ...
                    <div class="budget-auth-switch">
                        <button type="button" data-auth-target="login" @class([
                        ...
                        <button type="button" data-auth-target="register" @class([
                            'budget-auth-switch-button',
                    ...
                    <div class="mt-8">
                        <form method="POST" action="{{ route('login.store') }}" class="space-y-5"
                            data-auth-panel="login" @if ($authMode !== 'login') hidden @endif>
                            @csrf
                            <input type="hidden" name="auth_mode" value="login">

                            <div>
                                <h2 class="text-2xl font-bold tracking-tight text-slate-900">Masuk</h2>
                                <p class="budget-auth-copy">Gunakan email dan password untuk melanjutkan ke dashboard.
                                </p>
                            </div>
                            ...
                            <label class="budget-field">
                                ...
                                <input type="email" name="email"
                                    value="{{ old('auth_mode', 'login') === 'login' ? old('email') : '' }}"
                            ...
                            <label class="flex items-center gap-3 text-sm text-slate-600">
                                <input type="checkbox" name="remember" value="1" @checked(old('auth_mode', 'login') === 'login' && old('remember'))
                            ...
                            <p class="text-center text-sm text-slate-500">
                                Belum punya akun?
                                <button type="button" class="font-semibold text-blue-600"
                                    data-auth-target="register">Buat Akun</button>
                            </p>
                        </form>

                        <form method="POST" action="{{ route('register.store') }}" class="space-y-5"
                            data-auth-panel="register" @if ($authMode !== 'register') hidden @endif>
                            @csrf
                            <input type="hidden" name="auth_mode" value="register">

                            <div>
                                <h2 class="text-2xl font-bold tracking-tight text-slate-900">Daftar</h2>
                                <p class="budget-auth-copy">Buat akun baru dan langsung masuk ke dashboard budgeting
                                    Anda.</p>
                            </div>
                            ...
                            <p class="text-center text-sm text-slate-500">
                                Sudah punya akun?
                                <button type="button" class="font-semibold text-blue-600"
                                    data-auth-target="login">Masuk</button>
                            </p>
                        </form>
                    </div>
                </div>
